...
namespace {
	...
	private:
       ...
       class hashpbn_reporter : public hashpbn_tree_detail::damage_visitor {
	public:
		hashpbn_reporter(nested_output &out)
		: out_(out),
		  err_(NO_ERROR) {
		}

		virtual void visit(hashpbn_tree_detail::missing_hashes const &d) {
			out_ << "missing hashes: " << d.keys_ << end_message();
			{
				nested_output::nest _ = out_.push();
				out_ << d.desc_ << end_message();
			}
	...
	private:
       ...
       class lbnpbn_reporter : public lbnpbn_tree_detail::damage_visitor {
        public:
                lbnpbn_reporter(nested_output &out)
                : out_(out),
                  err_(NO_ERROR) {
                }

                virtual void visit(lbnpbn_tree_detail::missing_lbns const &d) {
                        out_ << "missing lbns: " << d.keys_ << end_message();
                        {
                                nested_output::nest _ = out_.push();
                                out_ << d.desc_ << end_message();
                        }
	...
	error_state metadata_check(string const &path, flags fs) {
		...
		superblock_reporter sb_rep(out);
                hashpbn_reporter hash_rep(out);               
                lbnpbn_reporter lbn_rep(out); 
		...
		transaction_manager::ptr tm = open_tm(bm);
                out << "examining hash to pbn tree" << end_message();
                {
                nested_output::nest _ = out.push();
	        hashpbn_tree dtree(*tm, sb.hash_pbn_root_,hashpbn_tree_detail::hashpbn_details_traits::ref_counter());
		check_hashpbn_tree(dtree, hash_rep);
                }
                out << "examining lbn to pbn tree" << end_message();
                {
                nested_output::nest _ = out.push();
                lbnpbn_tree dtree(*tm, sb.lbn_pbn_root_,lbnpbn_tree_detail::lbnpbn_details_traits::ref_counter());
                check_lbnpbn_tree(dtree, lbn_rep);
                }
		return combine_errors(sb_rep.get_error(),combine_errors(hash_rep.get_error(),lbn_rep.get_error()));
	}
...
int dmdedup_check_main(int argc, char **argv)
{
	int c;
	flags fs;
...
base::command dmdedup::dmdedup_check_cmd("dmdedup_check", dmdedup_check_main);
